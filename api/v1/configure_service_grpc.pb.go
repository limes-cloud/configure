...
type ServiceClient interface {
	...
	WatchConfig(ctx context.Context, in *WatchConfigRequest, opts ...grpc.CallOption) (Service_WatchConfigClient, error)
}
...
func (c *serviceClient) WatchConfig(ctx context.Context, in *WatchConfigRequest, opts ...grpc.CallOption) (Service_WatchConfigClient, error) {
	stream, err := c.cc.NewStream(ctx, &Service_ServiceDesc.Streams[0], Service_WatchConfig_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &serviceWatchConfigClient{stream}
...
type Service_WatchConfigClient interface {
	Recv() (*WatchConfigReply, error)
	grpc.ClientStream
}

type serviceWatchConfigClient struct {
	grpc.ClientStream
}

func (x *serviceWatchConfigClient) Recv() (*WatchConfigReply, error) {
	m := new(WatchConfigReply)
...
type ServiceServer interface {
	...
	WatchConfig(*WatchConfigRequest, Service_WatchConfigServer) error
...
func (UnimplementedServiceServer) WatchConfig(*WatchConfigRequest, Service_WatchConfigServer) error {
...
func _Service_WatchConfig_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(WatchConfigRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ServiceServer).WatchConfig(m, &serviceWatchConfigServer{stream})
}

type Service_WatchConfigServer interface {
	Send(*WatchConfigReply) error
	grpc.ServerStream
}

type serviceWatchConfigServer struct {
	grpc.ServerStream
}

func (x *serviceWatchConfigServer) Send(m *WatchConfigReply) error {
