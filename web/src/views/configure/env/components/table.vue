  ...
  >
    ...
    <template #operations="{ record }">
      <a-space class="cursor-pointer">
        <a-tag
          v-permission="'configure:environment:token:query'"
          color="arcoblue"
          @click="emit('queryToken', record.id)"
        >
          <template #icon><icon-edit /></template> 获取token
        </a-tag>

        <a-tag
          v-permission="'configure:environment:token:reset'"
          color="orangered"
          @click="emit('resetToken', record.id)"
        >
          <template #icon><icon-edit /></template> 重置token
        </a-tag>

        <a-tag
          v-permission="'configure:environment:update'"
          color="orangered"
